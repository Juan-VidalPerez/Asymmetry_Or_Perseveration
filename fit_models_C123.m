function [parametersLPP,LPP] = fit_models_C123(data,model,fit_procedure,varargin)
...
for k_sub = 1:subjecttot
        ...
        parfor k_rep = 1:n_rep
            [parametersLPP_rep(k_rep,:),LPP_rep(k_rep),~,~,~,~,FminHess(k_rep,:,:)]=fmincon(@(x) Parameters_Priors_Complete_Final(x,current_sta,current_cho,current_out,current_cou,current_obs,model,fit_procedure,phi_sd),x0(k_rep,:),[],[],[],[],LB,UB,[],options);
        end
        
        [~,posLPP] = min(LPP_rep);
        parametersLPP(k_sub,1:nfpm(model)) = parametersLPP_rep(posLPP(1),1:nfpm(model));
        if strcmp(fit_procedure,'MAP')
            LPP(k_sub) = LPP_rep(posLPP(1),:) - nfpm(model)*log(2*pi)/2 + real(log(det(squeeze(FminHess(posLPP(1),:,:)))))/2;
        elseif strcmp(fit_procedure,'MLE')
...
function [post]=Parameters_Priors_Complete_Final(params,s,a,r,c,obs,model,fit,phi_sd)
...
if strcmp(fit,'MAP')
    ...
    elseif model == 3
        ...
        phi   = params(5); pphi  = log(normpdf(phi,0,phi_sd));
    ...
    elseif model == 4
        ...
        phi   = params(6); pphi  = log(normpdf(phi,0,phi_sd));
