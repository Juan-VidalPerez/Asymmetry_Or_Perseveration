function [parametersLPP,LPP] = fit_models_noC123(data,model,fit_procedure, varargin)
...
for k_sub = 1:subjecttot
        ...
        parfor k_rep = 1:n_rep
            [parametersLPP_rep(k_rep,:),LPP_rep(k_rep),~,~,~,~,FminHess(k_rep,:,:)]=fmincon(@(x) Parameters_Priors_Complete_Final(x,current_sta,current_cho,current_out,current_cou,model,fit_procedure,phi_sd),x0(k_rep,:),[],[],[],[],LB,UB,[],options);
        end
...
function [post]=Parameters_Priors_Complete_Final(params,s,a,r,c,model,fit,phi_sd)
...
if strcmp(fit,'MAP')
    ...
    elseif model == 3
        ...
        phi   = params(4); pphi  = log(normpdf(phi,0,phi_sd));
