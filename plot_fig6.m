function plot_fig6(varargin)
% Generates plots for Figure 6, showing behavioral signatures generated by
% simulating models with varying levels of confirmation bias or perseveration.
%
% This function creates three figures:
% 1. Figure 6a (CB): Learning curves for CB parameter sweep.
% 2. Figure 6a (PERS): Learning curves for PERS parameter sweep.
% 3. Figure 6b: Interaction term coefficient from history analysis for
%    'Katahira' and 'Classic' tasks.
%
% USAGE:
%   plot_fig6(100);       % Use 100 simulations for Fig 6a, 100 datasets for Fig 6b
%   plot_fig6(50000, 1000);  % Use 50000 for Fig 6a, 1000 datasets for Fig 6b (as in paper)
%
% INPUTS:
%   varargin{1} (n_sims_a) - Sims per parameter value for Figure 6a plots.
%   varargin{2} (n_sims_b) - (Optional) Number of datasets (each with 20 agents)
%                            to simulate for Figure 6b plot.
%
% REQUIREMENTS:
...
close all;
fprintf('--- Generating Figure 6: Loading base parameters from MAP_fits.mat ---\n');

% Handle flexible n_sims input
if nargin == 0
    error('Please provide the number of simulations, e.g., plot_fig6(100).');
elseif nargin == 1
    n_sims_a = varargin{1};
    n_sims_b = varargin{1}; % Number of datasets for Fig 6b
else
    n_sims_a = varargin{1};
    n_sims_b = varargin{2}; % Number of datasets for Fig 6b
end
n_agents_per_dataset_b = 20; % Fixed number of agents per dataset in Fig 6b
...
%% 2. Plot Learning Curves (Separate Figures for CB and PERS - Figure 6a)
fprintf('--- Plotting Figure 6a (Learning Curves) ---\n');
...
for type_idx = 1:length(param_types)
    ...
    sgtitle(['Figure 6a (' current_type ')'], 'FontSize', 16, 'FontWeight', 'bold');
...
%% 3. Plot History Interaction Coefficient (Figure 6b)
fprintf('--- Plotting Figure 6b (Regression Analysis) ---\n');
fig_hist = figure('Position', [950, 100, 800, 700]); % Increased height for 2 rows
sgtitle('Figure 6b', 'FontSize', 16, 'FontWeight', 'bold');
...
end % End of main function plot_fig6
